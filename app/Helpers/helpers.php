...
if (! function_exists('getPercentage')) {
    function getPercentage($amount,$targetAmount) {
//        10rb :100rb x 100
        $percentage = ($amount / $targetAmount) * 100;
        return $percentage;
    }
}
...
if (! function_exists('createInvoice')) {
    function createInvoice($orderId,$prefix = 'INV')
    {
        $invoice = $prefix.'/'.date('Ymd').'/'.date('His').strTotime(date('Y-m-d H:i:s'));
...
if (! function_exists('createBooking')) {
    function createBooking($prefix = '')
    {
        if($prefix != '') {
            $prefix = $prefix.'-';
        }
        $booking = $prefix.strTotime(date('Y-m-d H:i:s'));
        return $booking;
    }
}

if (! function_exists('getDateDiffInDays')) {
...
if (! function_exists('getTotalDayFromDate')) {
    function getTotalDayFromDate($startDate,$endDate)
    {
        $to_date = strtotime($endDate);
        $from_date = strtotime($startDate);
        $day_diff = $to_date - $from_date;
        $result = floor($day_diff/(60*60*24));
        return $result;
    }
}
...
if (! function_exists('getDateIndoShort')) {
    /**
     * Generate new datetime from configured format datetime.
     *
     * @param  string $datetime
     * @return string
     */
    function getDateIndoShort($date)
    {
        switch(date("F", strtotime($date)))
        {
            case 'January':     $nmb="Jan";     break;
            case 'February':    $nmb="Feb";    break;
            case 'March':       $nmb="Mar";       break;
            case 'April':       $nmb="Apr";       break;
            case 'May':         $nmb="Mei";         break;
            case 'June':        $nmb="Jun";        break;
            case 'July':        $nmb="Jul";        break;
            case 'August':      $nmb="Agu";     break;
            case 'September':   $nmb="Sep";   break;
            case 'October':     $nmb="Okt";     break;
            case 'November':    $nmb="Nov";    break;
            case 'December':    $nmb="Des";    break;
        }
